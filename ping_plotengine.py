...
def PlotPing(host, list, maxdata):
    ...
    if version.parse(plt.__version__) > version.parse("5.3.2"):
        plt.clear_color()
    else:
        plt.clear_figure()   # removed ?  plt.clear_plot()
    plt.clt()
    ...
    if version.parse(plt.__version__) >= version.parse("5.3.2"):
        plt.clear_color()
    elif version.parse(plt.__version__) > version.parse("2.3.1"):  # in version 3.0.1, nocolor is changed to colorless()
        plt.colorless()
    else:
        plt.nocolor()
    plt.sleep(0.01)
